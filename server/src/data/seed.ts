import type { Approval, Client, Dashboard, Employee, Invoice, Role } from '../types.js';
...
export function dashboardFor(role: Role, pendingCount: number): Dashboard {
  ...
  if (role === 'hr') {
    return {
      ...
      pendingApprovals: Math.min(2, pendingCount),
      benchCount: 2,
      metrics: [
  ...
  return {
    ...
    metrics: [
      ...
      { key: 'approvals', label: 'Pending approvals', value: String(pendingCount), tone: 'warning' },
...
export function matchesQuery(q: string | undefined, ...fields: string[]) {
  if (!q) return true;
